...
use weir_wab::format::{
    self, ConfirmedMeta, EXT_CONFIRMED, EXT_SEALED, SegmentFooterMeta, SegmentHeaderMeta,
};
use weir_wab::{
    SegmentReader, SegmentState, SegmentVerifyError, list_segment_files, verify_sealed_segment,
};
...
impl From<std::io::Error> for WabError {
...
#[derive(Serialize)]
pub struct FooterJson {
    pub record_count: u64,
    pub data_bytes: u64,
    pub file_crc32: String,
    pub sealed_at: i64,
}
impl From<&SegmentFooterMeta> for FooterJson {
    fn from(ft: &SegmentFooterMeta) -> Self {
        FooterJson {
            record_count: ft.record_count,
            data_bytes: ft.data_bytes,
            file_crc32: format!("{:#010x}", ft.file_crc32),
            sealed_at: ft.sealed_at,
        }
    }
}
#[derive(Serialize)]
pub struct ConfirmedJson {
    pub sealed_at: i64,
    pub record_count: u64,
    pub drained_at: i64,
}
...
pub struct SegmentJson {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub footer: Option<FooterJson>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub integrity: Option<IntegrityJson>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub confirmed: Option<ConfirmedJson>,
}
...
fn header_of(path: &Path) -> Option<HeaderJson> {
    SegmentReader::open(path)
        .ok()
        .map(|r| HeaderJson::from(r.header()))
}
...
pub fn inventory(wab_dir: &Path) -> Result<SegmentsResponse, WabError> {
    ...
    if dl.is_dir() {
        totals.dead_letter = list_segment_files(&dl)
            .unwrap_or_default()
            .into_iter()
            .filter(|(_, st)| matches!(st, SegmentState::Sealed | SegmentState::Active))
            .count();
    }
    for (path, state) in list_all_segment_files(wab_dir)? {
        ...
        match state {
            SegmentState::Active => {
                totals.active += 1;
                seg.header = header_of(&path);
            }
            SegmentState::Sealed => {
                totals.sealed += 1;
                match verify_sealed_segment(&path) {
                    Ok(v) => {
                        seg.header = Some(HeaderJson::from(&v.header));
                        seg.footer = Some(FooterJson::from(&v.footer));
                        seg.integrity = Some(IntegrityJson::ok());
                    }
                    Err(e) => {
                        seg.header = header_of(&path);
                        seg.integrity = Some(IntegrityJson::from_err(&e));
                    }
                }
            }
            SegmentState::Confirmed => {
                ...
                if let Ok(buf) = std::fs::read(&path)
                    && let Ok(c) = format::parse_confirmed(&buf)
                {
                    seg.confirmed = Some(ConfirmedJson::from(&c));
                }
            }
        }
        segments.push(seg);
    ...
    let confirmed: Vec<(String, ConfirmedJson)> = segments
        .iter()
        .filter(|s| s.state == "confirmed")
        .filter_map(|s| {
            s.confirmed.as_ref().map(|c| {
                (
                    s.file.replace(EXT_CONFIRMED, EXT_SEALED),
                    ConfirmedJson {
                        sealed_at: c.sealed_at,
                        record_count: c.record_count,
                        drained_at: c.drained_at,
    ...
    for (sealed_file, c) in confirmed {
        if let Some(s) = segments.iter_mut().find(|s| s.file == sealed_file) {
            s.confirmed = Some(c);
        }
    }
    ...
    Ok(SegmentsResponse {
        wab_dir: wab_dir.to_string_lossy().into_owned(),
        totals,
        segments,
    })
}
...
pub fn records(
    ...
    offset: usize,
    limit: usize,
) -> Result<RecordsResponse, WabError> {
    ...
    Ok(RecordsResponse {
        ...
        header,
        records,
        terminated_cleanly: reader.terminated_cleanly(),
    })
}

#[derive(Serialize)]
pub struct DeadLetterSegmentJson {
    pub file: String,
    pub records: Vec<RecordJson>,
    pub terminated_cleanly: Option<bool>,
}
...
pub struct DeadLetterResponse {
    ...
    pub segments: Vec<DeadLetterSegmentJson>,
}
...
pub fn dead_letter(wab_dir: &Path) -> Result<DeadLetterResponse, WabError> {
    ...
    if dl_dir.is_dir() {
        for (path, _state) in list_segment_files(&dl_dir)? {
            ...
            segments.push(DeadLetterSegmentJson {
                file: rel,
                records: r.records,
                terminated_cleanly: r.terminated_cleanly,
            });
        }
    }
    Ok(DeadLetterResponse {
        dead_letter_dir: dl_dir.to_string_lossy().into_owned(),
        segments,
    })
}
...
pub fn verify(wab_dir: &Path, rel: &str) -> Result<IntegrityJson, WabError> {
    let path = safe_join(wab_dir, rel)?;
    Ok(match verify_sealed_segment(&path) {
        Ok(_) => IntegrityJson::ok(),
        Err(e) => IntegrityJson::from_err(&e),
    })
}
