...
fn inventory_reports_state_meta_and_integrity() {
    ...
    let sealed_ok = inv
        .segments
        .iter()
        .find(|s| s.file.ends_with("seg_00000001.wab.sealed"))
        .unwrap();
    ...
    let corrupt = inv
        .segments
        .iter()
        .find(|s| s.file.ends_with("seg_00000003.wab.sealed"))
        .unwrap();
    ...
    let rec_corrupt = inv
        .segments
        .iter()
        .find(|s| s.file.ends_with("seg_00000005.wab.sealed"))
        .unwrap();
    assert_eq!(
        rec_corrupt.integrity.as_ref().unwrap().kind.as_deref(),
        Some("BadRecord")
    );
    ...
    let active = inv
        .segments
        .iter()
        .find(|s| s.file.ends_with("seg_00000002.wab"))
        .unwrap();
...
fn records_reads_payloads_with_crc_and_termination() {
    ...
    assert!(matches!(
        wab::records(&root, "../etc/passwd", 0, 10),
        Err(wab::WabError::BadPath(_))
    ));
}
...
fn dead_letter_lists_payload_records() {
    ...
    assert_eq!(
        dl.segments[0].records[0].utf8_preview.as_deref(),
        Some("rejected-1")
    );
}
...
fn verify_returns_ok_and_structured_error() {
    ...
    let root = fixtures::build_fixtures(dir.path());
    assert!(
        wab::verify(&root, "shard_00/seg_00000001.wab.sealed")
            .unwrap()
            .ok
    );
...
async fn http_segments_ok_and_bad_path_400() {
    ...
    let resp = app
        .oneshot(
            Request::get("/api/wab/segment?path=../x&offset=0&limit=10")
                .body(Body::empty())
                .unwrap(),
        )
        .await
