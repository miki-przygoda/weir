...
test("explorer routes return real-shaped JSON incl. a corrupt segment", async () => {
  ...
  const vt = await j(f, "/api/wab/verify?path=shard_00/seg_00000004.wab.sealed");
  assert.equal(vt.kind, "MissingFooter");
});
...
test("ops drop empties the store; preview does not", async () => {
  const f = loadMock();
  assert.equal((await j(f, "/api/ops/dead-letter")).count, 6);
  const prev = await j(f, "/api/ops/drop/preview", { method: "POST" });
  assert.equal(prev.candidates, 6);
  assert.equal((await j(f, "/api/ops/dead-letter")).count, 6, "preview must not mutate");
  const done = await j(f, "/api/ops/drop", { method: "POST" });
  assert.equal(done.dropped, 6);
...
test("ops requeue empties the store and reports records", async () => {
  ...
  assert.equal(prev.requeuable_records, 340);
  const done = await j(f, "/api/ops/requeue?durability=sync", { method: "POST" });
  assert.equal(done.requeued_records, 340);
