...
| **D** io_uring | ❌ not shipped — **validated** | **Empirically confirmed not worth it.** A direct micro-benchmark on beast (96k records, 64/fsync, SATA SSD) compared `writev`+`fdatasync` vs io_uring (batched writes + IO_DRAIN datasync fsync). io_uring was **never faster across 4 runs** (−0.2%, −2.8%, −4.6%, −46.5%) — at best identical, at worst far worse. The ~2 ms `fdatasync` is the entire cost and io_uring can't speed it up; its ring/completion bookkeeping only adds overhead and jitter. `write_vectored` already captured the one real win (collapsing write syscalls), portably. io_uring is also Linux-only (absent on macOS) and adds unsafe complexity. Verdict: do not pursue. |
