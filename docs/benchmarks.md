...
| [drain-throughput.md](benchmarks/drain-throughput.md) | **Delivery-side** rates — how fast the buffer *empties* into an HTTP sink, per-record vs NDJSON, and under a slow sink. Every other row on this table is ingest. **Mixed basis:** the mac rows were re-measured 2026-09-15 on the corrected window; the two Linux rows still predate it and are marked superseded in place. Check the basis column before quoting a row. |
