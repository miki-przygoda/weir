...
- ~~**Fix the Batched deadline anomaly.**~~ **RECLASSIFIED — non-actionable (Stream A + code reading, 2026-06-11).** The scouts hypothesised a second independent `recv_timeout(batch_deadline)` in the flusher making Batched p99.9 (11.2 ms) worse than Sync (3.8 ms). Reading `flush_batch` disproves it: (1) **Sync and Batched are the same code path** — both set `need_fsync` and ride the single group-fsync, acking after it, so there is no tier-specific slow path; (2) the flusher's batch drain uses non-blocking `try_recv` (`wab/mod.rs:400-405`), not a second deadline wait. The CI p99.9 gap is tail noise on a contended 2-vCPU runner, already mitigated by Stream A's 500→2000 sample widening. No fix needed; Stream B is bridge-removal only. (Sync==Batched is a deliberate design choice — the group-fsync satisfies Sync's "durable before ack" contract — out of scope here.)
