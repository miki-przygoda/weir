...
- **If you do**, run it in addition. `run_vectors.py` checks both and reports
...
round-trips to the same bytes. The tracked extension has its own suite,
`cargo test -p weir-core --test conformance_tracked`, which also fails if a
tracked frame is ever added to the frozen v1 file.
...
It also checks `wire_v1_tracked_vectors.json` when that file is present, and
reports it on its own line — including a check that every tracked frame is
rejected as `UnknownMessageType` by the *v1-only* decoder, which is the
