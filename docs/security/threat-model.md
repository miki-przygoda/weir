...
| A batch reply that answers a different batch | `AckBatch` echoes the record count, and a client must check it against what it sent. `ceil(N/8)` is not injective — N of 1017 through 1024 all yield 131 bytes — so length alone cannot detect a desync, and a bitmap carries no other self-identifying field. |
