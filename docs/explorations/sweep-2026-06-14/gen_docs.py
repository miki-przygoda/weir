...
ew("> The findings from the sweep that I did **not** change autonomously: each needs a product/API call or a redesign. Most are 1.0-freeze decisions and several pair with the deferred freeze hooks (WAB-v2 headroom, reserved Nack byte, conformance vectors, `Sink::commit` signature). Nothing here is load-bearing-broken. Fixed items + queued-safe items live in [`FINDINGS.md`](FINDINGS.md).\n")
ew(f"**{len(escalated)} open decisions**, severity-ordered:\n")
for f in escalated:
    ew(f"- [{f['id']}](#{f['id'].lower()}--{re.sub(r'[^a-z0-9]+','-',f['title'].lower()).strip('-')}) — *({f['severity']})* {f['title']}")
ew("")
for f in escalated:
    anchor = re.sub(r"[^a-z0-9]+", "-", f["title"].lower()).strip("-")
