...
- **Fixed: 22** — incl. the one CRITICAL data-loss bug (F12), every high-severity bug, and the mediums.
- **Needs your decision: 10** → all in **[`ESCALATIONS.md`](ESCALATIONS.md)** (separate file so they're easy to find).
- **Queued safe fixes: 28** — being worked through; this list shrinks as they land.
