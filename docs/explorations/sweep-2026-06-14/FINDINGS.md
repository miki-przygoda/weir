...
> Output of the Max-tier multi-agent sweep (orthogonal lens + independent subsystem sweeps, completeness critic, per-finding adversarial verification). Plan: [`PLAN.md`](PLAN.md). Per-subsystem detail: [`subsystems/`](subsystems/). Raw data: `findings.json`. Every fix below is its own commit (grep the log for the `[Fxx]` tag) and was gated (fmt + clippy -D warnings + tests; DST 300-seed where durability-adjacent).

## TL;DR

- **76 raw** candidates → **60 confirmed-real** after adversarial verification (+ 1 uncertain, 1 refuted).
- **Fixed tonight: 22** — including the one **CRITICAL data-loss bug (F12)**, every **high-severity bug**, and most mediums.
- **Needs your decision / redesign: 10** — documented with a recommendation below; most are 1.0-freeze API calls. Nothing load-bearing is broken.
- **Queued safe fixes (not done tonight): 28** — localized low-risk items (docs, minor guards, extra tests) for a clean follow-up batch.
- Full gate green after all fixes: bin 300 (all features), workspace + doctests, DST 300-seed, clippy across default·tls·clickhouse·dst·no-default-features.

## 🔴 Critical — fixed

**F12 — recovery counter-reset overwrites a recovered segment (data loss).** `crates/weir-server/src/wab/segment.rs:335` · `fb02a62`. `file_stem()` stripped only the last extension, so the counter scan ignored `.wab.sealed`/`.wab.confirmed` files; after recovery sealed the active segment the scan reset `next_counter` to 1 and the next seal-rename clobbered a recovered-but-undrained sealed segment. Fixed by parsing the counter from the `seg_NNNNNNNN` prefix regardless of extension + regression tests + DST sweep. This is exactly the class of bug the prior big-bug-focused sweeps had masked.
...
Deliberately **not** changed tonight — each needs a product/API call or a redesign. Recommendation inline; most are 1.0-freeze decisions and several pair with the deferred freeze hooks.
...
➡️ Config-load `warn!`s (unknown TOML keys, the dead_letter <1MiB advisory) are discarded because the tracing subscriber is initialised AFTER `Config::load()` — so a TOML typo silently takes defaults. Fix needs either collecting the warnings and emitting them post-init, or a reloadable filter (bootstrap level → reload to config.log_level). Both are small refactors with a logging-init design choice. **Recommend:** collect-and-emit. Escalated for the approach call.
...
➡️ `QueueSender::len()` has no `is_empty()` (clippy::len_without_is_empty), latent only because there's no clippy lint config. Trivial + safe. **Recommend / queued-safe:** add `is_empty()`.
...
Localized, low-risk — a clean follow-up batch (the night ran out before these; none need a decision).
