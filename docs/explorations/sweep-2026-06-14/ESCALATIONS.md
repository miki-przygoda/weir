...
> The findings from the sweep that I did **not** change autonomously: each needs a product/API call or a redesign. Most are 1.0-freeze decisions and several pair with the deferred freeze hooks (WAB-v2 headroom, reserved Nack byte, conformance vectors, `Sink::commit` signature). Nothing here is load-bearing-broken. Fixed items + queued-safe items live in [`FINDINGS.md`](FINDINGS.md).

**10 open decisions**, severity-ordered:

- [F41](#f41--commitresult-partition-invariant-committed-dead-lettered-batch-is-unenforced-the-drain-confirms-deletes-the-segment-unconditionally-on-ok) — *(high)* CommitResult partition invariant (committed ∪ dead_lettered = batch) is unenforced; the drain confirms+deletes the segment unconditionally on Ok
- [F42](#f42--sinkrecord-into-payload-is-documented-as-the-dead-letter-recovery-path-but-is-bypassed-for-whole-batch-permanent-and-transient-errors) — *(high)* SinkRecord::into_payload is documented as the dead-letter recovery path but is bypassed for whole-batch permanent (and transient) errors
- [F05](#f05--retried-multi-batch-segment-re-dead-letters-earlier-sub-batches-amplifying-duplicate-dead-letter-files-and-thrashing-the-cap) — *(medium)* Retried multi-batch segment re-dead-letters earlier sub-batches, amplifying duplicate dead-letter files and thrashing the cap
- [F25](#f25--unknownmessagetype-unknowndurability-decode-errors-are-nacked-as-the-documented-as-transient-keep-connection-open-internalerror-reason-yet-the-connection-is-closed) — *(medium)* UnknownMessageType/UnknownDurability decode errors are nacked as the (documented-as-transient, keep-connection-open) InternalError reason, yet the connection is closed
- [F43](#f43--blocking-client-sets-no-socket-read-write-connect-timeouts-a-wedged-daemon-blocks-producers-indefinitely) — *(medium)* Blocking client sets no socket read/write/connect timeouts; a wedged daemon blocks producers indefinitely
- [F48](#f48--public-error-enums-and-commitresult-are-not-non-exhaustive-adding-any-variant-field-after-1-0-is-a-breaking-change) — *(medium)* Public error enums and CommitResult are not #[non_exhaustive]; adding any variant/field after 1.0 is a breaking change
- [F54](#f54--config-time-warn-calls-are-silently-dropped-tracing-subscriber-initialized-after-config-load) — *(medium)* Config-time warn! calls are silently dropped (tracing subscriber initialized after Config::load)
- [F50](#f50--header-new-takes-a-payload-len-argument-that-envelope-new-always-overwrites-an-api-that-invites-desync) — *(low)* Header::new takes a payload_len argument that Envelope::new always overwrites — an API that invites desync
- [F24](#f24--queuesender-exposes-len-with-no-is-empty) — *(info)* QueueSender exposes len() with no is_empty()
- [F52](#f52--reserved-flags-byte-is-preserved-verbatim-on-decode-but-never-validated-to-be-zero-foreclosing-in-version-flag-evolution) — *(info)* Reserved flags byte is preserved verbatim on decode but never validated to be zero, foreclosing in-version flag evolution
