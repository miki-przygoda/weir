...
Shard directories and the quarantine directory are created with mode `0o700`. Segment files are created with mode `0o600`.
...
`file_crc32` is accumulated by a running `crc32fast::Hasher` during writes, so no full-file re-read is needed at seal time.
...
A sidecar with bad magic, unknown version, or CRC mismatch causes both the sidecar and its sealed segment to be quarantined.
