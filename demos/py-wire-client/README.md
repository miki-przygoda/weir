...
`tests/test_conformance.py` runs the codec against all **28 canonical vectors**
(17 valid frames + 11 rejection cases). For every vector it checks the decode
...
python3 demos/py-wire-client/tests/test_conformance.py     # -> "28/28 vectors passed"
...
| `tests/test_conformance.py`   | Runs the codec against the 28 canonical vectors (decode + fields for all, byte-exact re-encode for valid frames); plain script. |
